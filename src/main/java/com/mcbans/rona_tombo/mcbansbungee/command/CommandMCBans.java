...
import com.mcbans.rona_tombo.mcbansbungee.callback.ManualResync;
import com.mcbans.rona_tombo.mcbansbungee.callback.ManualSync;
...
import com.mcbans.rona_tombo.mcbansbungee.request.PingRequest;
...
public class CommandMCBans extends BaseCommand{
	...
	public void run() throws CommandException{
		/* General Help */
		if(args.size() == 0){
			send("&bMCBans &3 BungeeCord Edition " + plugin.getDescription().getVersion() + "&b Help &f|| &b<> &f= required, &b[] &f= optional");
		...
		if(first.equalsIgnoreCase("banning")){
			...
			send("&f/rban <name|uuid> [reason]" + ChatColor.RED + " This command cannnot use MCBans BungeeCord Edition");
			send("&f/rban <name|uuid> -g <reason>" + ChatColor.RED + " This command cannnot use MCBans BungeeCord Edition");
			send("&f/rban <name|uuid> -t <time> <m, h, d, w> <reason>" + ChatColor.RED + " This command cannnot use MCBans BungeeCord Edition");
		...
		if(first.equalsIgnoreCase("staff") && player != null && plugin.mcbStaff.contains(player.getName())){
			if(args.size() > 0 && args.get(0).equalsIgnoreCase("perms")){
				...
				send("&6-=== Debug Information (BungeeCord Edition) ===-");
				send("&6BungeeCord Version: &e" + plugin.getProxy().getVersion());
				...
				send("&6timeRecieved: &e" + plugin.timeReceived + " &6syncRunning: &e" + plugin.syncRunning + " &6lastID: &e" + plugin.lastID);
				...
				send("&6Failsafe: &e" + config.isFailedSafe() + "&6 isDebug: &e" + config.isDebug() + "&6 Log: &e" + config.isEnableLog());
